...
| 4 | Comparative synthesis | Claude generates structured JSON (or a degraded plain-text fallback) |

Successful runs **persist a synthesis trace** to Postgres (same observability pattern as chat), when `DATABASE_URL` is configured.

### Async jobs and polling

Long-running work cannot block the HTTP response for tens of seconds. The server uses:

- **`POST /api/compare-sources`** — Validates the request body, creates an **in-memory job** keyed by UUID, starts **`runCompareSourcesAsync`** without awaiting it, and returns **`{ jobId }`** immediately.
